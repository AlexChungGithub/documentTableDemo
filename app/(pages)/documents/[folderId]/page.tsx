...
import { TagsIcon, FlagIcon, ArrowRightIcon } from "lucide-react";
...
function fakeList() {
  ...
  // priority menu
  const priorityList = ["Low", "Meduim", "High"].map((val, index) => ({
    id: index.toString(),
    name: val
  }));
  ...
  const result = res
    ...
    .map((val) => {
      return {
        ...val,
        priority:
          priorityList.find((data) => data.id === val.priority)?.name || "",
        category:
        ...
        {selectIdList.length === 0 ? (
          ...
          >
            ...
            <CheckboxMenu
              ...
              clearCallback={() => {
                dispatch({ type: "FILTER_MENU", payload: { priority: [] } });
              }}
              selectedCallback={(selected) => {
                dispatch({
                  type: "FILTER_MENU",
            ...
            <Dialog.Root>
              ...
              <Dialog.Content style={{ maxWidth: 450 }}>
                ...
                <Flex direction="column" gap="3">
                  <label>
                    <TextField.Input placeholder="Create new tag..." />
                  </label>
                </Flex>
              ...
              >
                <Checkbox
                  ...
                  onClick={(e) => {
                    ...
                    if (isClicked) {
                      const newIds = checkArrayMatch(
                        selectIdList,
                        idList
                      ).exclude;
                      setSelectIdList([...selectIdList, ...newIds]);
                    } else {
                      const newIds = selectIdList.filter(
              ...
              files.map((item) => {
                return (
                  <TableRow
                    ...
                    key={item.id}
                    item={item}
                    selectIdListState={[selectIdList, setSelectIdList]}
...
function TableRow({
  ...
  const Priority = ({ ...props }: { children: React.ReactNode }) => {
    ...
    if (children === "High") {
      result = (
        <Badge variant="solid" color="red" radius="large">
          {children}
        </Badge>
      );
    } else {
  ...
  return (
    <Table.Row
      style={{
        backgroundColor: selectIdList.includes(item.id)
          ? "Highlight"
          : undefined
      }}
    >
